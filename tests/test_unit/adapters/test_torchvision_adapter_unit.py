"""Unit tests for TorchVisionAdapter.

Stub transforms are used to test protocol compliance, matrix shape, flip dims, and parameter sampling without invoking
real torchvision transforms. Skipped entirely when torchvision is not installed.
...
@pytest.fixture
def _register_stubs(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_torchvision_adapter_exported_from_adapters_package():
    ...
    assert _PublicTorchVisionAdapter is not None
...
class TestSampleParamsFlips:
    ...
    @pytest.mark.parametrize(
        "stub_cls",
        [
            pytest.param(_StubHFlip, id="hflip"),
            pytest.param(_StubVFlip, id="vflip"),
        ],
    )
...
class TestBuildMatrixIdentity:
    ...
    def test_identity_affine_params(self, adapter):
        ...
        assert torch.allclose(matrix, expected, atol=1e-5), (
            f"Expected identity, max diff: {(matrix - expected).abs().max().item():.2e}"
        )
...
class TestBuildMatrixHFlip:
    ...
    def test_hflip_first_row(self, adapter):
        ...
        for idx in range(batch_size):
            assert matrix[idx, 0, 0].item() == pytest.approx(-1.0)
            assert matrix[idx, 0, 1].item() == pytest.approx(0.0)
            assert matrix[idx, 0, 2].item() == pytest.approx(float(width - 1))
...
class TestBuildMatrixVFlip:
    ...
    def test_vflip_second_row(self, adapter):
        ...
        for idx in range(batch_size):
            assert matrix[idx, 1, 0].item() == pytest.approx(0.0)
            assert matrix[idx, 1, 1].item() == pytest.approx(-1.0)
            assert matrix[idx, 1, 2].item() == pytest.approx(float(height - 1))
...
class TestBuildMatrixFallback:
    ...
    def test_unregistered_transform_returns_identity(self, adapter):
        ...
        matrix = adapter.build_matrix(object(), {}, 64, 64)
        assert matrix.shape == (1, 3, 3), f"Expected shape (1, 3, 3), got {matrix.shape}"
