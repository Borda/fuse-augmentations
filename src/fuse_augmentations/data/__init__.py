"""Synthetic detection / segmentation / OBB dataset generation.

Draw colored shapes on a canvas and export **COCO** or **YOLO** datasets for
detection, segmentation, or oriented-bounding-box tasks. This is a standalone
...
def generate_dataset(
    ...
    **config_kwargs: Any,  # noqa: ANN401 - forwarded verbatim to SyntheticConfig
    ...
    Args:
        ...
        task: ``"detection"``, ``"segmentation"``, or ``"obb"`` (or a :class:`Task`).
        ...
        config: Full :class:`SyntheticConfig`; when given, ``class_mode`` and
            ``config_kwargs`` are ignored in favor of the config's own fields.
