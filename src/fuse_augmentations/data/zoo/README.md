...
This "16-point" schema is specific to the `AnimalShape` family loaded from this directory. The other two keypoint-bearing families ship their own assets in their own directories and use their own schemas: `../symbols/` holds one outline document per symbol (7 points), and `../letters/` holds one *stroke-graph* document per letter (15 grid nodes, no outline — the silhouette is generated by stroking the graph). All three are read by the same parser, `fuse_augmentations.data.svgio`, and edited by the same tool, so the authoring workflow below carries over; only the point vocabulary differs. See `docs/guides/synthetic-datasets.md` for each family's schema.
