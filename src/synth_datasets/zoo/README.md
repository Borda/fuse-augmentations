...
This "16-point" schema is specific to the `AnimalShape` family loaded from this directory. The other two keypoint-bearing families ship their own assets in their own directories and use their own schemas: `../symbols/` holds one outline document per symbol (7 points), and `../letters/` holds one *stroke-graph* document per letter (15 grid nodes, no outline — the silhouette is generated by stroking the graph). All three are read by the same parser, `synth_datasets.svgio`, and edited by the same tool, so the authoring workflow below carries over; only the point vocabulary differs. See `docs/datasets/tasks.md` for each family's schema.

## The 16-point anatomical topology

Fixed and shared across every animal in this package — one dataset-wide `kpt_shape` for YOLO, one `ANIMAL_KEYPOINT_NAMES`/`ANIMAL_KEYPOINT_SKELETON` pair for COCO. Defined once in `synth_datasets.animals`, alongside the `AnimalShape` roster and the loader that reads these files; this file only explains how to *place* points against it, it does not redefine it. One schema covers quadrupeds, birds, and swimmers alike: a front limb is whatever the animal actually has at that slot — a paw, a wing, or a fin/flipper.
...
The colors are **fixed per name and identical across all twelve animals**, so once you know that blue is always the neck and magenta always the tail, you can read any file at a glance. The library itself never reads a fill, so the palette lives with the tool that writes it — `ANIMAL_PALETTE` in `examples/edit_shape_keypoints.py`. A test instead checks these documents agree with themselves: one color per landmark name across all twelve files, and no two names sharing one.
...
- `<path id="outline">` — straight-line-only (`M`/`L`/`Z`, absolute or relative, `H`/`V` accepted). No curves, no `transform` on any element — see `synth_datasets.animals` for the exact parser contract and rejection messages.
...
5. **Register + verify**: add one member to the `AnimalShape` enum in `animals.py` — the roster, the keypoint-capable shape list, and `ANIMAL_NAMES` all derive from it — then pin its absent-keypoint set in `tests/test_unit/test_synth_datasets/test_animals.py` (`ABSENT_KEYPOINTS`), then run `pytest tests/test_unit/test_synth_datasets` — the suite checks the SVG parses, the provenance is present, every keypoint lies inside the rasterized silhouette (rotated and not), no two present points coincide, and the absent set matches the pin.

## Editing keypoints by hand

Two ways, both fine:

- **Interactive editor** (fastest): `python examples/edit_shape_keypoints.py <animal>` — press and hold a colored dot to drag it, release to drop, press `s` to save back into the SVG (skeleton lines are regenerated automatically), `q` to quit. A color/name legend sits beside the canvas; running it without a name lists every shape in every family it can edit.
...
Every file's root `<svg>` carries `zoo:origin` (source URL), `zoo:license`, `zoo:attribution`, and `zoo:title` (the source species' scientific name) as namespaced attributes — see `synth_datasets.animals` for the parser that validates these are present.
